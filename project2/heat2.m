...
for elem = 1:Mesh.nElem % loop over elements in the mesh
  ...
  % Find x and y coordinates of each node 
  x1 = x(1,1);
  x2 = x(2,1);
  x3 = x(3,1);
  y1 = x(1,2);
  ...
  % determine k
  if Mesh.Elem2Material(elem,1) == 1
      k = 40;
  else
      k = 2.5;
  end
  ...
  %
  K(I(1), I(1)) = K(I(1), I(1)) + dot(k*[dp1dx,dp1dy],[dp1dx,dp1dy]*A);
  K(I(1), I(2)) = K(I(1), I(2)) + dot(k*[dp1dx,dp1dy],[dp2dx,dp2dy]*A);
  K(I(1), I(3)) = K(I(1), I(3)) + dot(k*[dp1dx,dp1dy],[dp3dx,dp3dy]*A);
  
  K(I(2), I(1)) = K(I(2), I(1)) + dot(k*[dp2dx,dp2dy],[dp1dx,dp1dy]*A);
  K(I(2), I(2)) = K(I(2), I(2)) + dot(k*[dp2dx,dp2dy],[dp2dx,dp2dy]*A);
  K(I(2), I(3)) = K(I(2), I(3)) + dot(k*[dp2dx,dp2dy],[dp3dx,dp3dy]*A);
  
  K(I(3), I(1)) = K(I(3), I(1)) + dot(k*[dp3dx,dp3dy],[dp1dx,dp1dy]*A);
  K(I(3), I(2)) = K(I(3), I(2)) + dot(k*[dp3dx,dp3dy],[dp2dx,dp2dy]*A);
  K(I(3), I(3)) = K(I(3), I(3)) + dot(k*[dp3dx,dp3dy],[dp3dx,dp3dy]*A);
...
for edge = 1:Mesh.nEdge % loop over boundary edges
  ...
  ds = sqrt((x(1,1)-x(2,1))^2 + (x(1,2)-x(2,2))^2);
  m = [1/3 1/6; 1/6 1/3];
  
  iBC = Mesh.BC(edge,3);
  if (iBC == 0)     % hot gas side
    
     F(I) = F(I) + T_hot*.5*ds*h_hot;
     K(I,I) = + K(I,I) + h_hot*ds*m;
     
    
  elseif (iBC == 1) % cold gas side
    
     F(I) = F(I) + T_cool*.5*ds*h_cool;
     K(I,I) = + K(I,I) + h_cool*ds*m;
